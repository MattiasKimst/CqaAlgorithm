...
public class InconsistenciesInserter {
    ...
    public static void insertInconsistencies(Database database, int numberOfFactsToBeDuplicated,
    ...
    private static List<Fact> getRandomFacts(List<Fact> factsInRelation, int numberOfFactsToBeSelected) {
        List<Fact> copy = new ArrayList<>(factsInRelation);
        Collections.shuffle(copy);
        return copy.subList(0, numberOfFactsToBeSelected);
    }

    public static List<Fact> duplicateFacts(List<Fact> factsToBeDuplicated, int numberOfTimesToBeDuplicated) throws Exception {
